...
##### 3. 指令範例 (v1.6.0 - Smart Workspace)
部署完成後，小B具備了「路徑感知」與「智慧存檔」能力，您只需直接對小B下令：
*   **智慧存檔**：「幫我把這份重點存到 `專案/2024報告.doc`。」(自動建立資料夾並產生文檔)
*   **表格生成**：「搜集台積電股價並存成 `投資/股價表.sheet`。」(自動產生試算表)
*   **網頁下載**：「下載這個網址的 PDF 並存到 `下載/財報.pdf`: [URL]。」(自動後台下載)
*   **路徑瀏覽**：「幫我列出 `小B專用` 資料夾裡的檔案。」

> [!TIP]
> **建議模型**: 為確保「路徑解析」與「邏輯判斷」的精準度，強烈建議將模型設定為 `gemini-2.5-pro`。
